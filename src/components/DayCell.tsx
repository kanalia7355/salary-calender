...
export default function DayCell({ day, dateKey: _dateKey, entries, settings, isToday, isSelected, dayOfWeek, onClick }: Props) {
  ...
  return (
    <div
      className={`h-20 rounded p-1 cursor-pointer ${cellBg} ${border} hover:bg-gray-700 transition-colors`}
      onClick={onClick}
    >
      <div className={`text-sm font-medium ${textColor}`}>{day}</div>
      {hasEntries && (
        <div className="mt-1 space-y-0.5">
          <div className="bg-green-700 rounded px-1 py-0.5 text-xs text-green-100 truncate">
            {entries.length === 1 ? entries[0].projectName : `${entries.length}件`}
          </div>
          <div className="text-xs text-green-400 font-medium">¥{totalPay.toLocaleString()}</div>
        </div>
      )}
    </div>
  );
